...
func TestListenVariantsAndUnsupported(t *testing.T) {
	...
	memLis, err := Listen("mem://")
	if err != nil {
		t.Fatalf("listen mem: %v", err)
	}
	if memLis.Addr().String() != "mem://" {
		t.Fatalf("mem addr = %q, want mem://", memLis.Addr().String())
	}
	_ = memLis.Close()
...
func TestMemDialContextAndAddr(t *testing.T) {
	mem := NewMemListener()
	defer mem.Close()
	...
	clientConn, err := mem.DialContext(context.Background(), "mem")
	if err != nil {
	...
	case <-time.After(1 * time.Second):
		t.Fatal("timeout waiting for accepted mem connection")
	}

	if got := mem.Addr().String(); got != "mem://" {
		t.Fatalf("mem addr = %q, want mem://", got)
	}
}
